$(document).ready(function() {
  ...
  if ($(projectListSelector).length) {
    ...
    $.get(mainContestLink).then(mainProjectPageData => {
      var timelineObj = $('[class*="side_panel__timelineContainer"]', mainProjectPageData).first();
      ...
      $(projectListSelector).each(function() {
        var projectObj = this;
        var isPrivate = $('h4 > [data-original-title="Private"]', this).length > 0;
        // if it's private, accessing the project will throw a 401 error
        if (!isPrivate) {
